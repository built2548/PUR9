...
public class Enemy_Script : MonoBehaviour
{
    ...
    [SerializeField] private int pointsOnDeath = 500;
...
private void Die()
{
    if (isDead) return;
    isDead = true; 
    // ⭐ NEW: Award points to the player
    ...
    // 3. ⭐ LAYER & COLLIDER LOGIC (Option 1)
    // Switch to a custom layer so bullets pass through, but Ground still stops the fall
    // Ensure you create the "DeadEnemy" layer in Unity first!
    int deadLayer = LayerMask.NameToLayer("DeadEnemy");
    if (deadLayer != -1) // Check if the layer exists
    {
        gameObject.layer = deadLayer;
    }
    else
    {
        // Fallback to Ignore Raycast if you haven't made the layer yet
        gameObject.layer = LayerMask.NameToLayer("Ignore Raycast");
    }

    // Adjust collider so the dead body rests properly on the ground
    myCapsule.offset = new Vector2(0f, 0.7f); 

    // 4. Physics: Stop horizontal movement but let gravity handle the vertical fall
